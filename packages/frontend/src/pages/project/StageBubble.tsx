...
const ActiveRipple: FC<{ color: string }> = ({ color }) => (
  <Box
    sx={{
      position: 'absolute',
      inset: -14,
...
const FinishBurst: FC<{ color: string; visible: boolean }> = ({
  color,
  visible,
}) => (
  <Box
...
const StageIcon: FC<{ status: StageStatus; color: string }> = ({
  status,
  color,
}) => {
...
export const StageBubble: FC<StageBubbleProps> = ({
  stage,
  statusLabels,
  celebrate,
  compact,
  ...
  const shouldFloat = isActive || (celebrate && stage.key === 'delivery');
  ...
  return (
    ...
    >
      ...
      >
        ...
        <FinishBurst color={accent} visible={celebrate && stage.key === 'delivery'} />
      </Box>
      <Typography
