...
- `ProjectStage`: init → pending → progress → done
...
- Парсинг stdout для получения JSON метаданных
...
#### 1.3 ✅ Настроить TypeScript конфигурацию

**Создать**: [packages/backend-workers/tsconfig.json](packages/backend-workers/tsconfig.json) с конфигурацией TypeScript (базовые настройки наследуются от tsconfig.base.json)

**Обновить**: [packages/backend-workers/package.json](packages/backend-workers/package.json)

- Зависимости: `typescript`, `@types/node`
- Скрипты: `"build": "tsc"`, `"check:ts": "tsc --noEmit"`

**Обновить**: [tsconfig.json](tsconfig.json) - добавить reference: `{"path": "./packages/backend-workers"}`
...
- Использовать `CI_CHECK=true yarn` для установки только линтеров
...
- CI mode (`CI_CHECK=true yarn`): только dev зависимости (линтеры)
